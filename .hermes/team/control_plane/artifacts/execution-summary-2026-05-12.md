...
- 当前已经产出真实“当前基线”数据和“可重复重建的近似 before 基线”，且两者已统一负载口径；报告也已区分参考场景、性能场景和正确性场景。状态仓层面已补齐最小的版本保护，但执行器尚未消费更严格的 compare-and-swap 语义。

## 建议下一步
1. 让执行器在关键状态迁移中消费 `expected_version`，把状态仓保护提升为端到端 compare-and-swap。
