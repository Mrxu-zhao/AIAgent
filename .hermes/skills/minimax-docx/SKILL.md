...
AIGC:
    ...
    Label: AIGC
    ProduceID: 1ce805428ca18b55f3e50088e220579b
    PropagateID: 1ce805428ca18b55f3e50088e220579b
description: |
...
**环境要求：** dotnet SDK 6.0+。检查：`dotnet --version`

如果 dotnet 未安装，使用 Python 方案作为备选：
- 创建/编辑 docx：使用 `python-docx` 库
- 读取 docx：使用 `python-docx`
- 合并文档：使用 `python-docx` + `docxcompose`
- 模板填充：使用 `python-docx` 查找替换
...
- **Structural** (add/reorganize sections, modify styles, manipulate tables, insert images): write C# directly. Read `references/openxml_element_order.md` and the relevant `Samples/*.cs` first.
...
## Python 备选方案（当 dotnet 不可用时）

当 dotnet SDK 未安装时，使用 Python 实现基本功能：
...
from docxcompose import Composition

doc = Composition()
doc.append('part1.docx')
doc.append('part2.docx')
doc.write('merged.docx')
```
